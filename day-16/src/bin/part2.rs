...
fn process_input(input: &str) -> usize {
    ...
    (1..map[0].len() - 1).for_each(|x| {
        beams.push(Beam::new((x, 1), Direction::Down));
        beams.push(Beam::new((x, map.len() - 2), Direction::Up));
    });
    (1..map.len() - 1).for_each(|y| {
        beams.push(Beam::new((1, y), Direction::Right));
        beams.push(Beam::new((map[0].len() - 2, y), Direction::Left));
    });
    ...
    beams
        ...
        .map(|beam| {
            ...
            cache.iter().map(|b| b.pos).collect::<HashSet<_>>().len()
        })
        .max()
        .unwrap()
}
...
mod test {
    ...
    fn test_input() {
        ...
        assert_eq!(process_input(input), 51);
    }
}
