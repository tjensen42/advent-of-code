...
impl Hand {
    fn best_hand_type(cards: &[Card; 5]) -> HandType {
        ...
        if joker_count > 0 {
            ...
            for card in Card::iter() {
                (0..joker_count).for_each(|i| cards[i] = card);
                let hand_type = Self::identify_hand_type(&cards);
...
impl FromStr for Hand {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        ...
        Ok(Hand {
            hand_type: Self::best_hand_type(&cards),
            cards,
            bid,
        })
    }
